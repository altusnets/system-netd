...
/* From Android N, We are going to follow AOSP's location for vendor modules
  * MTK module will no longer be installed into /system
  */
#define AOSP_LOCATION_FOR_VENDOR 1
#ifdef AOSP_LOCATION_FOR_VENDOR
static char RADVD_PATH[] = "/system/vendor/bin/radvd";
static char DHCP6S_PATH[] = "/system/vendor/bin/dhcp6s";
#else
static char RADVD_PATH[] = "/system/bin/radvd";
static char DHCP6S_PATH[] = "/system/bin/dhcp6s";
#endif

#ifdef  IPV6_TETHER_NDP_MODE
#define NDP_MODE    1
#else
#define RADVD_MODE  2
	#ifdef IPV6_TETHER_PD_MODE
	#define PD_MODE		3
	#endif
#endif
...
int IPv6TetherController::setDefaults() {
	//don't flush routing table for L version
    mCount = 0;
    return 0;
}
...
int IPv6TetherController::setAllMulticast(bool set, const char *intIface, const char *extIface){
    if(intIface == NULL || extIface == NULL)
		return -1;
    if(set)
        ALOGD("setAllMulticast set == true");
...
int IPv6TetherController::findInfaceNumber(const char *iface){
	...
	while(iface_list[index] != 0){
		if (0 == strncmp(iface, iface_list[index],2))
			return index;
		else
			index++;
	}
...
int IPv6TetherController::startDhcp6s(const char *intIface) {
   ...
   if ((pid = fork()) < 0) {
    ...
    if (!pid) {
        if (execl(DHCP6S_PATH, DHCP6S_PATH,
...
int IPv6TetherController::startRadvd() {
   ...
   if ((pid = fork()) < 0) {
    ...
    if (!pid) {
        if (execl(RADVD_PATH,RADVD_PATH,
                  "-n", "-d 5", (char *) NULL)) {
            ALOGE("execl failed (%s)", strerror(errno));
        }
        ALOGE("Should never get here!");
...
int IPv6TetherController::setIPv6SecondaryRoute(const int argc, char **argv) {
    ...
    char subnet[64] = {'\0'};
	char clat6_addr[64] = {'\0'};
	const char *clat6if = "clat";
    ...
    snprintf(clat6_addr, sizeof(clat6_addr), "%s464/128", prefix_prop_value);
    ALOGD("generate clat6 address: %s", clat6_addr);
    ...
    if (tableNumber != -1) {
        ...
        snprintf(cmd, sizeof(cmd), "-6 route add %s dev %s table %d", subnet, intIface,
                    tableNumber);
        ret |= runCmd(IP_PATH, cmd);

        snprintf(cmd, sizeof(cmd), "-6 route add %s dev %s table %d", clat6_addr, clat6if,
                    tableNumber);
		//ignore error because there may be no clat6 interface
        runCmd(IP_PATH, cmd);
...
int IPv6TetherController::enableIPv6Tether(const int argc, char **argv) {
    ...
    if (startRadvd() != 0) {
        if(errno == EBUSY){
            ...
            updateRadvdConf();
            PerfController::enable_perf_rps(intIface, extIface);
            return 0;
        } else {
...
int IPv6TetherController::disableIPv6Tether(const int argc, char **argv) {
    ...
    PerfController::disable_perf_rps(intIface, extIface);
    return 0;
}
