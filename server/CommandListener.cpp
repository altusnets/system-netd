...
CommandListener::CommandListener() :
    ...
    registerLockingCmd(new Ipv6FwdCmd());
    registerLockingCmd(new IPv6TetherCmd());
    ...
    if (!strcmp(argv[1], "list")) {
        ...
        if (!strcmp(argv[1], "getcfg")) {
            ...
            if (!inet_aton(argv[3], &addr)) {
                ...
                if (inet_pton(AF_INET6, argv[3], &addr6)) {
            ...
            // arglist: iface
            ALOGD("[InterfaceCmd] Clearing all IP addresses on %s", argv[2]);

            ifc_clear_addresses(argv[2]);

            ALOGD("[InterfaceCmd] IP addresses on %s cleared", argv[2]);
...
int CommandListener::BandwidthControlCmd::runCommand(SocketClient *cli, int argc, char **argv) {
    ...
    if (!strcmp(argv[1], "setglobalalert") || !strcmp(argv[1], "sga")) {
        ...
        if((gCtls->perfCtrl.get_lowpower_perfhandle() != -1) && (gCtls->perfCtrl.get_tether_perfhandle() == -1)) {
            ALOGI("throughput exceeds limit, ignore setglobalalert cmd");
            return 0;
        }
...
int CommandListener::FirewallCmd::runCommand(SocketClient *cli, int argc,
    ...
    if (!strcmp(argv[1], "set_pkg_uid_rule")) {
        ...
        int res = gCtls->firewallCtrl.setPkgUidRule(uid, rule);
        return sendGenericOkFail(cli, res);
    }
    ...
    if (!strcmp(argv[1], "set_egress_proto_rule")) {
        ...
        int res = 0;
        res |= gCtls->firewallCtrl.setEgressProtoRule(proto, rule);
        return sendGenericOkFail(cli, res);
    }
...
int CommandListener::NetworkCommand::runCommand(SocketClient* client, int argc, char** argv) {
    ...
    if (!strcmp(argv[1], "fwmark")) {
        ...
        if (!strcmp(argv[2], "add")) {
            if (int ret = gCtls->netCtrl.setFwmarkPolicyRule(argv[3], atoi(argv[4]), true)) {
                return operationError(client, "setFwmarkPolicyRule()=true failed", ret);
            }
        } else if (!strcmp(argv[2], "remove")) {
            if (int ret = gCtls->netCtrl.setFwmarkPolicyRule(argv[3], atoi(argv[4]), false)) {
    ...
    if (!strcmp(argv[1], "permission")) {
        ...
        if (userPermissions) {
            ALOGD("[NetworkCommand] setPermissionForUsers");
            gCtls->netCtrl.setPermissionForUsers(permission, ids);
            ALOGD("[NetworkCommand] setPermissionForUsers return");
        } else {
            // networkPermissions
...
int CommandListener::Ipv6FwdCmd::runCommand(SocketClient *cli,
    ...
    if (!strcmp(argv[1], "status")) {
        ...
        asprintf(&tmp, "Forwarding %s", (gCtls->ipv6tetherCtrl.getIpv6FwdEnabled() ? "enabled" : "disabled"));
        cli->sendMsg(ResponseCode::IpFwdStatusResult, tmp, false);
        free(tmp);
        return 0;
    } else if (!strcmp(argv[1], "enable")) {
#ifdef TETHERINGIPV6_SUPPORT
        rc = gCtls->ipv6tetherCtrl.setIpv6FwdEnabled(true);
#else
        ALOGW("IPv6 tethering feature is disabled, ingore this command");
#endif
    } else if (!strcmp(argv[1], "disable")) {
#ifdef TETHERINGIPV6_SUPPORT
        rc = gCtls->ipv6tetherCtrl.setIpv6FwdEnabled(false);
#else
        ALOGW("IPv6 tethering feature is disabled, ingore this command");
#endif
    } else {
...
#ifdef TETHERINGIPV6_SUPPORT
    if (!strcmp(argv[1], "enable")) {
        rc = gCtls->ipv6tetherCtrl.enableIPv6Tether(argc, argv);
    } else if (!strcmp(argv[1], "disable")) {
        rc = gCtls->ipv6tetherCtrl.disableIPv6Tether(argc, argv);
        } else if (!strcmp(argv[1], "setroute")) {
                rc = gCtls->ipv6tetherCtrl.setIPv6SecondaryRoute(argc, argv);
        } else if (!strcmp(argv[1], "clearroute")) {
                rc = gCtls->ipv6tetherCtrl.clearIPv6SecondaryRoute(argc, argv);
        } else if (!strcmp(argv[1], "setsroute")) {
                rc = gCtls->ipv6tetherCtrl.setSourceRoute(argc, argv);
        } else if (!strcmp(argv[1], "clearsroute")) {
                rc = gCtls->ipv6tetherCtrl.clearSourceRoute(argc, argv);
        } else if (!strcmp(argv[1], "add")) {
                rc = gCtls->ipv6tetherCtrl.startDhcp6s(argv[2]);
        } else if (!strcmp(argv[1], "remove")) {
                rc = gCtls->ipv6tetherCtrl.stopDhcp6s(argv[2]);
        } else {
...
int CommandListener::NetInfoCmd::runCommand(SocketClient *cli, int argc, char **argv) {
    ...
    if (!strcmp(argv[1], "setsip")) {
        ...
        rc = gCtls->netInfo.setSipInfo(argc, argv);
    } else if (!strcmp(argv[1], "getsip")){
                rc = gCtls->netInfo.getSipInfo(argv[2],argv[3],argv[4], cli);
                return 0;
    } else if (!strcmp(argv[1], "clearsip")){
        rc = gCtls->netInfo.clearSipInfo(argv[2]);
        } else {
...
int CommandListener::ThroughputCmd::runCommand(SocketClient *cli,
    ...
    if (!rc) {
        cli->sendMsg(ResponseCode::CommandOkay, "throughput operation succeeded", false);
    } else {
        cli->sendMsg(ResponseCode::OperationFailed, "throughput operation failed", false);
    }
